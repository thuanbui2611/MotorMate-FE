...
export default function BlogCategory() {
  ...
  const [categoryDeleted, setCategoryDeleted] = useState<Category>(
    {} as Category
  );
  ...
  const openConfirmDeleteDiaglog = (blogCategory: Category) => {
    setConfirmDeleteDiaglog((cur) => !cur);
    setCategoryDeleted(blogCategory);
  };
  const cancelConfirmDeleteDiaglog = () => setConfirmDeleteDiaglog(false);

  async function handleDeleteBlogCategory(blogCategoryDeleted: Category) {
    await dispatch(deleteBlogCategoryAsync(blogCategoryDeleted.id));
  }
  const handleSelectBlogCategory = (
    actionName: string,
  ...
  return (
    <>
      <Breadcrumb pageName="Blog Category" />
      <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <div className="flex justify-end">
          <button
            onClick={() => handleSelectBlogCategory("Add new category")}
            type="button"
                      ...
                      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                        <div className="flex items-center space-x-3.5">
                          ...
                          <button
                            ...
                            onClick={() => openConfirmDeleteDiaglog(category)}
                          >
                            <svg
                              className="fill-current"
                              width="18"
      ...
      {confirmDeleteDiaglog && (
        <ConfirmDeleteDialog
          objectName={categoryDeleted.name}
          actionDelete={() => handleDeleteBlogCategory(categoryDeleted)}
