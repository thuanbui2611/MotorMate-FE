import {
  ...
  export const updateStatusAsync = createAsyncThunk<Vehicle, { vehicleId: string; statusNumber: number }>(
    "vehiclesPending/updateStatusAsync",
    async ( { vehicleId, statusNumber }, ThunkAPI) => {
      try {
        const response = await agent.Vehicle.updateStatusVehicle(vehicleId, statusNumber);
        return response;
      } catch (error: any) {
        return ThunkAPI.rejectWithValue({ error: error.data });
      }
  ...
  export const VehiclePendingSlice = createSlice({
    ...
    extraReducers: (builder) => {
      ...
      builder.addCase(updateStatusAsync.fulfilled, (state, action) => {
        ...
        vehiclesPendingAdapter.removeOne(state, vehicleId);
        toast.success("Update status of vehicle susccessfully!");
      })
