...
import { ModelVehicle, ModelVehicleParams } from "../../app/models/ModelVehicle";
...
function getAxiosParams(modelVehicleParams: ModelVehicleParams) {
  ...
  export const updateModelVehicleAsync = createAsyncThunk<ModelVehicle, FieldValues>(
    "modelVehicle/updateModelVehicleAsync",
    async (data, ThunkAPI) => {
      try {
        console.log("Updating modelVehicle...", data);
  ...
  export const ModelVehicleSlice = createSlice({
    ...
    reducers: {
      setModelVehicleParams: (state, action) => {
        state.modelVehicleParams = { ...state.modelVehicleParams, ...action.payload };
      },
